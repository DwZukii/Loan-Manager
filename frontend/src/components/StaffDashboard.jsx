...
import { Bug, ClipboardList, PenLine, BookOpen, LogOut, Menu, X, Lightbulb, MessageSquare, Search, Bell, Cake, CheckCircle2, Inbox, Phone } from 'lucide-react'
...
Balas YA jika berminat`;
  ...
  const renderLeadsTab = () => {
    if (selectedLead && activeTab === 'leads') {
      ...
      const backFn = () => { navigateTo(activeTab, null); setCurrentNote(''); setShowWaMenu(false); setIsSmsOpen(false); setIsEditingSmsScript(false); setSelectedFile(null); }
      return (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-5">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Phone Call</p>
                  <a
                    href={getCallUrl(currentLead.phone_number)}
                    onClick={() => handleStatusChange(currentLead.id, 'Called')}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">SMS</p>
                    <button
                      onClick={() => setIsSmsOpen(!isSmsOpen)}
                      className="text-xs font-bold text-slate-500 hover:text-slate-800 transition"
                    >
                      {isSmsOpen ? 'Hide ▲' : 'Show options ▼'}
                    </button>
                  </div>
                  <button
                    onClick={() => setIsSmsOpen(!isSmsOpen)}
                  ...
                  {isSmsOpen && (
                    <div className="mt-4 space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  ...
                  {showWaMenu && (
                    <div className="mt-4 space-y-3 bg-green-50 p-4 rounded-xl border border-green-100 animate-in fade-in slide-in-from-top-2">
                      {/* App toggle */}
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                  ...
                  {currentLead.document_url ? (
                    <div className="flex items-center justify-between gap-4 bg-blue-50 p-4 rounded-xl border border-blue-100">
                      <a href={currentLead.document_url} target="_blank" rel="noreferrer" className="text-blue-600 font-bold hover:underline flex items-center gap-2 text-sm">
                        📎 View File
                      </a>
