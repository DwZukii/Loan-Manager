...
export default function App() {
  ...
  if (userRole === 'agent') {
    return <StaffDashboard userEmail={userEmail} onLogout={handleLogout} />
  }
...
function AdminDashboard({ userEmail, userRole, onLogout }) {
  ...
  const [uploadSet, setUploadSet] = useState('Set A') // NEW: Upload Set Selection
  const [uploadStatus, setUploadStatus] = useState('')
  
  const [unassignedCounts, setUnassignedCounts] = useState({ 'Set A': 0, 'Set B': 0, 'Set C': 0 }) // NEW: Separated counts
  const [agentStats, setAgentStats] = useState([])
  const [assignEmail, setAssignEmail] = useState('')
  const [assignAmount, setAssignAmount] = useState('50')
  const [assignSet, setAssignSet] = useState('Set A') // NEW: Distribution Set Selection
  ...
  const fetchAdminData = async () => {
    ...
    if (statsData) {
      let counts = { 'Set A': 0, 'Set B': 0, 'Set C': 0 }
      const statsMap = {}
  ...
  const handleUploadToDatabase = async () => {
    if (!selectedPhoneCol || excelData.length === 0) return; setUploadStatus(`Pushing numbers to ${uploadSet}...`)
    const leadsToInsert = excelData.map(row => {
      ...
      return { phone_number: cleanPhone, assigned_to: 'unassigned', status: 'Pending', agent_notes: '', document_url: null, is_reviewed: false, lead_set: uploadSet }
    }).filter(lead => lead.phone_number.length >= 10 && lead.phone_number.length <= 15)
    
    const { error } = await supabase.from('leads').insert(leadsToInsert)
    if (!error) { setUploadStatus(`Success! Added ${leadsToInsert.length} numbers to ${uploadSet}.`); setExcelData([]); fetchAdminData() }
    else { setUploadStatus(`Error: ${error.message}`) }
  }

  const handleAssignLeads = async () => {
    if (!assignEmail || !assignAmount) return setAssignStatus("Please select a staff member and an amount.")
    const finalAmount = Math.min(parseInt(assignAmount), unassignedCounts[assignSet])
    if (finalAmount <= 0) return setAssignStatus(`No unassigned leads available in ${assignSet}.`)
    
    setAssignStatus(`Assigning ${assignSet} leads...`)
    const { data: leadsToAssign } = await supabase.from('leads').select('id').eq('assigned_to', 'unassigned').eq('lead_set', assignSet).limit(finalAmount)
    const ids = leadsToAssign.map(lead => lead.id)
    const { error } = await supabase.from('leads').update({ assigned_to: assignEmail }).in('id', ids)
    if (!error) { setAssignStatus(`Successfully assigned ${finalAmount} leads from ${assignSet} to ${assignEmail}.`); setAssignAmount('50'); setAssignEmail(''); fetchAdminData() }
  }

  const handleClearPool = async () => {
    if (window.confirm(`WARNING: Are you sure you want to delete ALL unassigned numbers in ${assignSet}? This cannot be undone.`)) {
      const { error } = await supabase.from('leads').delete().eq('assigned_to', 'unassigned').eq('lead_set', assignSet); 
      if (!error) { alert(`The ${assignSet} Pool has been completely cleared.`); fetchAdminData() }
    }
  }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-0">
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={uploadSet} onChange={(e) => setUploadSet(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-lg bg-white mb-2 font-bold text-gray-700">
              ...
              {columns.length > 0 && (
                <div className="p-5 bg-blue-50 border border-blue-100 rounded-xl mt-4">
                  ...
                  <button onClick={handleUploadToDatabase} className="w-full bg-blue-600 text-white font-bold py-2.5 rounded-lg hover:bg-blue-700 mt-4 shadow-sm">Push to {uploadSet}</button>
                </div>
              )}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="bg-gray-50 rounded-lg p-3 mb-4 mt-3 border border-gray-200 flex flex-col gap-2">
            <div className="flex justify-between items-center text-sm">
              <span className="text-gray-600">Set A Pool: <span className="font-bold text-blue-600 ml-1">{unassignedCounts['Set A']}</span></span>
              <span className="text-gray-600">Set B Pool: <span className="font-bold text-blue-600 ml-1">{unassignedCounts['Set B']}</span></span>
              <span className="text-gray-600">Set C Pool: <span className="font-bold text-blue-600 ml-1">{unassignedCounts['Set C']}</span></span>
            </div>
          </div>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={assignSet} onChange={(e) => setAssignSet(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-lg bg-white font-bold text-gray-700">
            ...
            <div className="flex gap-2">
              <button onClick={handleAssignLeads} className="flex-1 bg-green-600 text-white font-bold py-2.5 rounded-lg hover:bg-green-700 mt-2 shadow-sm">Assign Numbers</button>
              {userRole === 'super_admin' && unassignedCounts[assignSet] > 0 && (
                <button onClick={handleClearPool} className="mt-2 px-4 border border-red-200 text-red-500 rounded-lg text-sm font-bold hover:bg-red-50 transition">Clear Set</button>
              )}
            </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative z-0">
        <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
          <span>🔔</span> Recent Staff Activity & Notes
        </h2>
  ...
  const renderAnalyticsTab = () => {
    return (
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h2 className="text-xl font-bold text-gray-800 mb-4">{userRole === 'super_admin' ? "Global Staff Data Matrix" : "Team Data Matrix"}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
            ...
            {userRole === 'super_admin' ? (
              ...
              <div className="col-span-2 flex items-center bg-blue-50 px-4 rounded-xl text-sm text-blue-700 font-medium">
                Staff created here auto-assign to your team.
              </div>
            )}
          </div>
                  ...
                  return (
                    <div key={m.id} className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden flex flex-col">
                      <div className="bg-indigo-50 border-b border-indigo-100 p-4">
                        ...
                        <span className="text-indigo-600 text-xs font-black mt-1 uppercase tracking-wide">{team.length} Staff</span>
                      </div>
                      <div className="p-4 flex-1">
                        {team.length === 0 ? <p className="text-sm text-gray-400 italic">No staff assigned.</p> : (
            ...
            <div className="overflow-y-auto max-h-96 pr-2">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50 sticky top-0 border-y border-gray-200 z-10">
                  <tr>
                    <th className="p-3 font-semibold text-gray-600 text-sm">Staff Account</th>
...
// STAFF DASHBOARD 
// ==========================================
function StaffDashboard({ userEmail, onLogout }) {
  ...
  // NEW: Securely coded Promo Script
    ...
    <nav className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-40">
      <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <h1 className="text-xl font-extrabold text-indigo-950 tracking-tight">Tele Manager <span className="text-blue-600 text-sm font-bold ml-1 uppercase tracking-widest">STAFF</span></h1>
        ...
        <div className="flex items-center gap-4">
          <span className="text-xs font-bold text-gray-400 hidden md:block">{userEmail}</span>
          <button onClick={onLogout} className="bg-white border border-gray-200 text-red-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-50 transition shadow-sm">Sign Out</button>
        </div>
      </div>
      <div className="sm:hidden flex px-4 pb-2 space-x-2">
        <button onClick={() => {setActiveTab('leads'); setSelectedLead(null)}} className={`flex-1 py-1.5 text-xs font-bold rounded-lg ${activeTab === 'leads' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 bg-gray-50'}`}>My Leads</button>
        <button onClick={() => {setActiveTab('tutorial'); setSelectedLead(null)}} className={`flex-1 py-1.5 text-xs font-bold rounded-lg ${activeTab === 'tutorial' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 bg-gray-50'}`}>Tutorial</button>
      </div>
    </nav>
  )
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 sm:p-12">
            ...
            <div className="space-y-8">
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 mb-2">Review your pending numbers</h3>
                  <p className="text-gray-600 leading-relaxed">Your manager will assign numbers to your account. On the "My Leads" tab, you will see a list of numbers marked as "Pending".</p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 mb-2">Make the call or Send Promo</h3>
                  <p className="text-gray-600 leading-relaxed">Click "Details" on a lead. You can either call them directly, or click "Send WhatsApp Promo" to instantly open a chat with the official company promotional script pre-written.</p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 mb-2">Update the Status & Notes</h3>
                  <p className="text-gray-600 leading-relaxed">After speaking with them, change the status dropdown to Accepted, Rejected, or Thinking. If they need follow-up, type the details into the "Staff Notes" box and hit Save. Your manager will automatically see your notes.</p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 mb-2">Upload Signed Documents</h3>
                  <p className="text-gray-600 leading-relaxed">If a client agrees and signs a form, use the file upload section at the bottom of the lead details to attach their PDF or Image (Max 2MB). This sends it straight to management.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 p-4 sm:p-8 animate-in slide-in-from-right-8 duration-300">
          <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-md p-6 border border-gray-100">
              ...
              {/* RESTORED SEPARATE BUTTONS & NEW WHATSAPP PROMO */}
              <div className="flex flex-col gap-3 mb-8 pb-8 border-b border-gray-100">
                <div className="flex gap-3">
                  <a href={`tel:${currentLead.phone_number}`} className="flex-1 bg-blue-600 text-white text-center py-3.5 rounded-xl font-bold hover:bg-blue-700 shadow-sm transition">Call Now</a>
                  <a href={whatsappLink} target="_blank" rel="noreferrer" className="flex-1 bg-white border border-gray-200 text-gray-700 text-center py-3.5 rounded-xl font-bold hover:bg-gray-50 shadow-sm transition">Open in WhatsApp</a>
                </div>
                <a href={`https://wa.me/${currentLead.phone_number}?text=${encodeURIComponent(promoScript)}`} target="_blank" rel="noreferrer" className="w-full bg-green-50 text-green-700 border border-green-200 text-center py-3 rounded-xl text-sm font-bold hover:bg-green-100 transition flex justify-center items-center gap-2">
