...
import { createClient } from '@supabase/supabase-js' // <--- NEW: For the Ghost Connection
...
function AdminDashboard({ userEmail, userRole, onLogout }) {
  ...
  // --- NEW: Handle Account Creation via Ghost Connection ---
  const handleCreateAccount = async () => {
    if (!newAccEmail || !newAccPassword || newAccPassword.length < 6) {
      setAccCreateStatus("Email and password (min 6 chars) required.")
      return
    }
    ...
    try {
      // Create a ghost connection that doesn't save to the browser session
      ...
      // 1. Create the secure login in the Auth Vault
      ...
      // 2. Logic: Managers can only create Agents. Super Admin can do anything.
      ...
      // 3. Register them in the profiles directory
      const { error: profileError } = await supabase.from('profiles').insert([{
        email: newAccEmail,
        role: roleToAssign,
        manager_email: managerToAssign
      }])
      ...
      setNewAccEmail('')
      setNewAccPassword('')
      fetchAdminData() // Refresh the dashboard instantly
    } catch (err) {
      ...
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Assigned Numbers</h3>
            {isProfileLoading ? <p className="text-gray-500 text-center py-8">Loading leads...</p> : agentProfileLeads.length === 0 ? <p className="text-gray-500 text-center py-8">No numbers assigned.</p> : (
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={onLogout} className="bg-red-500 text-white px-4 py-2 rounded-lg font-medium hover:bg-red-600 transition shadow-sm">
            Sign Out
          </button>
        </div>

        {/* --- NEW SECTION: TEAM MANAGEMENT & ACCOUNT CREATION --- */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-8 relative z-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* Box 1: Create New Account (Visible to both) */}
            <div className="col-span-1 border border-gray-200 rounded-xl p-4 bg-gray-50">
              ...
              <div className="space-y-3">
                ...
                {/* Super Admin gets to choose Role and Manager */}
            ...
            {/* Box 2 & 3: Company Organization (Super Admin Only) */}
            {userRole === 'super_admin' && (
              <>
            ...
            {/* Placeholder to keep layout clean for managers */}
            {userRole === 'manager' && (
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {leads.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-10 text-center border border-gray-100">
            <div className="text-4xl mb-3">📭</div><p className="text-gray-500 font-medium">You have no numbers assigned to you yet.</p>
          </div>
        ) : (
          <>
            <div className="space-y-4">
